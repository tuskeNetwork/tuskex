...
package tuskex.common.setup;

import ch.qos.logback.classic.Level;
import tuskex.common.UserThread;
import tuskex.common.app.AsciiLogo;
...
public class CommonSetup {
    ...
    private static void setupLog(Config config) {
        String logPath = Paths.get(config.appDataDir.getPath(), "tuskex").toString();
        Log.setup(logPath);
    ...
    protected static void maybePrintPathOfCodeSource() {
        try {
            ...
            if (!pathOfCodeSource.endsWith("classes"))
                log.info("Path to Tuskex jar file: " + pathOfCodeSource);
