...
dpkg -i "/home/amnesia/Persistent/tuskex_1.0.7-1_amd64.deb"
echo -e "Allowing amnesia to read tor control port cookie, only run this script when you actually want to use tuskex\n\n!!! not secure !!!\n"
chmod o+r /var/run/tor/control.authcookie
echo "Updating apparmor-profile"
echo "---
- apparmor-profiles:
    - '/opt/tuskex/bin/Tuskex'
  users:
    - 'amnesia'
  commands:
    AUTHCHALLENGE:
...
echo "alias tuskex-tails='torsocks /opt/tuskex/bin/Tuskex --torControlPort 951 --torControlCookieFile=/var/run/tor/control.authcookie --torControlUseSafeCookieAuth --useTorForTsk=ON --userDataDir=/home/amnesia/Persistent/'" >> /home/amnesia/.bashrc
echo -e "Everything is set up just run\n\nsource ~/.bashrc\n\nThen you can start tuskex using tuskex-tails"
