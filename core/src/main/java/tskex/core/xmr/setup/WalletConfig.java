...
import static tuskex.common.util.Preconditions.checkDir;
...
public class WalletConfig extends AbstractIdleService {
    ...
    protected volatile File vTskWalletFile;
    ...
    protected TskLocalNode tskLocalNode;
    ...
    public WalletConfig setTskLocalNode(TskLocalNode tskLocalNode) {
        checkState(state() == State.NEW, "Cannot call after startup");
    ...
    public void maybeAddSegwitKeychain(Wallet wallet, KeyParameter aesKey) {
        if (TuskexKeyChainGroupStructure.BIP44_BTC_NON_SEGWIT_ACCOUNT_PATH.equals(wallet.getActiveKeyChain().getAccountPath())) {
            ...
            try {
                FileUtil.copyFile(new File(directory, "tuskex_BTC.wallet"), backup);
            } catch (IOException e) {
            ...
            DeterministicKeyChain nativeSegwitKeyChain = DeterministicKeyChain.builder().seed(seed)
                    ...
                    .accountPath(new TuskexKeyChainGroupStructure().accountPathFor(Script.ScriptType.P2WPKH)).build();
            if (aesKey != null) {
