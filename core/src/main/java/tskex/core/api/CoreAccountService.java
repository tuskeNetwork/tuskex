...
import tuskex.common.util.ZipUtils;
...
public class CoreAccountService {
        ...
        PersistenceManager.flushAllDataToDiskAtBackup(() -> {
            try {
                ...
                List<File> excludedFiles = Arrays.asList(
                        new File(TskWalletService.MONERO_WALLET_RPC_PATH),
                        new File(TskLocalNode.MONEROD_PATH)
                );
    ...
    public void deleteAccount(Runnable onShutdown) {
        try {
            ...
            File dataDir = new File(config.appDataDir.getPath()); // TODO (woodser): deleting directory after gracefulShutdown() so services don't throw when they try to persist (e.g. TskTxProofService), but gracefulShutdown() should honor read-only shutdown
