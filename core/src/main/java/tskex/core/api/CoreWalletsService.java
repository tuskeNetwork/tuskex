...
import tuskex.core.api.model.BtcBalanceInfo;
import tuskex.core.api.model.TskBalanceInfo;
import tuskex.core.app.AppStartupState;
import tuskex.core.user.Preferences;
import tuskex.core.util.FormattingUtils;
import static tuskex.core.util.ParsingUtils.parseToCoin;
...
import static tuskex.core.tsk.wallet.Restrictions.getMinNonDustOutput;
...
class CoreWalletsService {
    ...
    BalancesInfo getBalances(String currencyCode) {
        ...
        switch (currencyCode.trim().toUpperCase()) {
            case "":
            case "TSK":
                return new BalancesInfo(BtcBalanceInfo.EMPTY, getTskBalances());
            default:
    ...
    String getTskSeed() {
        return tskWalletService.getWallet().getSeed();
    }

    String getTskPrimaryAddress() {
        return tskWalletService.getWallet().getPrimaryAddress();
    }

    String getTskNewSubaddress() {
        accountService.checkAccountOpen();
        return tskWalletService.getNewAddressEntry().getAddressString();
    }

    List<MoneroTxWallet> getTskTxs() {
        accountService.checkAccountOpen();
        return tskWalletService.getTxs();
    }

    MoneroTxWallet createTskTx(List<MoneroDestination> destinations) {
        accountService.checkAccountOpen();
        verifyWalletsAreAvailable();
        verifyEncryptedWalletIsUnlocked();
        try {
            return tskWalletService.createTx(destinations);
        } catch (Exception ex) {
            log.error("", ex);
    ...
    String relayTskTx(String metadata) {
        accountService.checkAccountOpen();
        verifyWalletsAreAvailable();
        verifyEncryptedWalletIsUnlocked();
        try {
            return tskWalletService.getWallet().relayTx(metadata);
        } catch (Exception ex) {
            log.error("", ex);
            throw new IllegalStateException(ex);
        }
    ...
    // Throws a RuntimeException if wallet currency code is not BTC or TSK.
    ...
    private TskBalanceInfo getTskBalances() {
        verifyWalletsAreAvailable();
        verifyEncryptedWalletIsUnlocked();
