...
import tuskex.core.offer.availability.OfferAvailabilityProtocol;
...
public class Offer implements NetworkPayload, PersistablePayload {
    ...
    public BigInteger getReservedAmount() {
        ...
        return TuskexUtils.tskWalletService.getOutputsAmount(offerPayload.getReserveTxKeyImages());
    }
    ...
    public BigInteger getMakerFee(BigInteger tradeAmount) {
        return TuskexUtils.multiply(tradeAmount, getMakerFeePct());
    }

    public BigInteger getTakerFee(BigInteger tradeAmount) {
        return TuskexUtils.multiply(tradeAmount, getTakerFeePct());
    }
    ...
    public boolean isTskAutoConf() {
        if (!isTsk()) {
            return false;
        }
        if (getExtraDataMap() == null || !getExtraDataMap().containsKey(OfferPayload.TSK_AUTO_CONF)) {
            return false;
        }

        return getExtraDataMap().get(OfferPayload.TSK_AUTO_CONF).equals(OfferPayload.TSK_AUTO_CONF_ENABLED_VALUE);
    }

    public boolean isTsk() {
