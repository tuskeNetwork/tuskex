...
import tuskex.common.util.CollectionUtils;
import tuskex.common.util.Hex;
import tuskex.common.util.JsonExclude;
...
public final class OfferPayload implements ProtectedStoragePayload, ExpirablePayload, RequiresOwnerIsOnlinePayload {
    ...
    // For traditional offer the baseCurrencyCode is TSK and the counterCurrencyCode is the traditional currency
    // For crypto offers it is the opposite. baseCurrencyCode is the crypto and the counterCurrencyCode is TSK.
    ...
    // Comma separated list of ordinal of a tuskex.common.app.Capability. E.g. ordinal of
    // Capability.SIGNED_ACCOUNT_AGE_WITNESS is 11 and Capability.MEDIATION is 12 so if we want to signal that maker
    // of the offer supports both capabilities we add "11, 12" to capabilities.
    public static final String CAPABILITIES = "capabilities";
    // If maker is seller and has tskAutoConf enabled it is set to "1" otherwise it is not set
    public static final String TSK_AUTO_CONF = "tskAutoConf";
    public static final String TSK_AUTO_CONF_ENABLED_VALUE = "1";
    ...
    // Fiat offers have base currency TSK and counterCurrency Fiat
    // Cryptos have base currency Crypto and counterCurrency TSK
    ...
    public BigInteger getMaxMakerFee() {
        return TuskexUtils.multiply(BigInteger.valueOf(getAmount()), getMakerFeePct());
    }
    ...
    public BigInteger getBuyerSecurityDepositForTradeAmount(BigInteger tradeAmount) {
        BigInteger securityDepositUnadjusted = TuskexUtils.multiply(tradeAmount, getBuyerSecurityDepositPct());
    ...
    public BigInteger getSellerSecurityDepositForTradeAmount(BigInteger tradeAmount) {
        BigInteger securityDepositUnadjusted = TuskexUtils.multiply(tradeAmount, getSellerSecurityDepositPct());
