...
import tuskex.core.offer.placeoffer.tasks.MakerProcessSignOfferResponse;
import tuskex.core.offer.placeoffer.tasks.MakerReserveOfferFunds;
import tuskex.core.offer.placeoffer.tasks.MakerSendSignOfferRequest;
    ...
    public void handleSignOfferResponse(SignOfferResponse response, NodeAddress sender) {
        ...
        TaskRunner<PlaceOfferModel> taskRunner = new TaskRunner<>(model,
                () -> {
                    ...
                    stopTimeoutTimer();
                    resultHandler.handleResult(model.getTransaction()); // TODO (woodser): TSK transaction instead
                },
                (errorMessage) -> {
