    ...
    public boolean isValid(String keyAndToken) {
        ...
        if (magic.equals(MobileModel.OS.IOS.getMagicString()) ||
            ...
            if (phoneId.length() != 64) {
                log.error("invalid Tuskex MobileModel ID format: iOS token not 64 bytes");
                return false;
            }
        } else if (magic.equals(MobileModel.OS.ANDROID.getMagicString())) {
            if (phoneId.length() < 32) {
                log.error("invalid Tuskex MobileModel ID format: Android token too short (<32 bytes)");
                return false;
            }
        } else {
            log.error("invalid Tuskex MobileModel ID format");
            return false;
        }
