...
import tuskex.core.support.dispute.DisputeResult.Winner;
...
import tuskex.core.support.dispute.mediation.FileTransferReceiver;
...
public final class ArbitrationManager extends DisputeManager<ArbitrationDisputeList> implements MessageListener, FileTransferSession.FtpCallback {
    ...
    public ArbitrationManager(P2PService p2PService,
        ...
        TuskexUtils.arbitrationManager = this; // TODO: storing static reference, better way?
    ...
    protected String getDisputeInfo(Dispute dispute) {
        ...
        String link = "https://docs.tuskex.exchange/trading-rules.html#legacy-arbitration";
                    ...
                    if (!trade.isPayoutPublished() && disputeClosedMessage.getUnsignedPayoutTxHex() != null) {
                        ...
                        if (disputeClosedMessage.isDeferPublishPayout()) {
                            ...
                            for (int i = 0; i < 5; i++) {
                                ...
                                TuskexUtils.waitFor(Trade.DEFER_PUBLISH_MS / 5);
                            }
                            if (!trade.isPayoutPublished()) trade.syncAndPollWallet();
                        }
...
//    BigInteger tradeAmount = BigInteger.valueOf(contract.getTradeAmount().value).multiply(ParsingUtils.TSK_SATOSHI_MULTIPLIER);
        ...
        if (trade.getPayoutTxHex() == null) {
            ...
            if (feeEstimateTx != null) {
                ...
                if (feeDiff > TskWalletService.MINER_FEE_TOLERANCE) throw new RuntimeException("Miner fee is not within " + (TskWalletService.MINER_FEE_TOLERANCE * 100) + "% of estimated fee, expected " + feeEstimate + " but was " + arbitratorSignedPayoutTx.getFee());
