...
public class DisputeValidation {
    ...
    public static void validateDisputeData(Dispute dispute) throws ValidationException {
        try {
            ...
            if (makerContractSignature != null) TuskexUtils.verifySignature(contract.getMakerPubKeyRing(),  dispute.getContractAsJson(), makerContractSignature);
            byte[] takerContractSignature = dispute.getTakerContractSignature();
            if (takerContractSignature != null) TuskexUtils.verifySignature(contract.getTakerPubKeyRing(),  dispute.getContractAsJson(), takerContractSignature);
        } catch (Throwable t) {
