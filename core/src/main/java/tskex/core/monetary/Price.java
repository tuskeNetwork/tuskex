...
public class Price extends MonetaryWrapper implements Comparable<Price> {
    ...
    public Volume getVolumeByAmount(BigInteger amount) {
        if (monetary instanceof TraditionalMoney)
            return new Volume(new TraditionalExchangeRate((TraditionalMoney) monetary).coinToTraditionalMoney(TuskexUtils.atomicUnitsToCoin(amount)));
        else if (monetary instanceof CryptoMoney)
            return new Volume(new CryptoExchangeRate((CryptoMoney) monetary).coinToCrypto(TuskexUtils.atomicUnitsToCoin(amount)));
        else
    ...
    public BigInteger getAmountByVolume(Volume volume) {
        ...
        if (monetary instanceof TraditionalMoney && this.monetary instanceof TraditionalMoney)
            return TuskexUtils.coinToAtomicUnits(new TraditionalExchangeRate((TraditionalMoney) this.monetary).traditionalMoneyToCoin((TraditionalMoney) monetary));
        else if (monetary instanceof CryptoMoney && this.monetary instanceof CryptoMoney)
            return TuskexUtils.coinToAtomicUnits(new CryptoExchangeRate((CryptoMoney) this.monetary).cryptoToCoin((CryptoMoney) monetary));
        else
