...
public class ProcessInitMultisigRequest extends TradeTask {
    ...
    protected void run() {
        try {
          ...
          TskWalletService tskWalletService = processModel.getProvider().getTskWalletService();

          // get sender
          TradePeer sender = trade.getTradePeer(processModel.getTempTradePeerNodeAddress());

          // set trade fee address
          if (TuskexUtils.ARBITRATOR_ASSIGNS_TRADE_FEE_ADDRESS) {
            ...
            trade.getProcessModel().setTradeFeeAddress(TuskexUtils.getGlobalTradeFeeAddress());
          }
          ...
          if (trade.getSelf().getMadeMultisigHex() == null && peers[0].getPreparedMultisigHex() != null && peers[1].getPreparedMultisigHex() != null) {
            ...
            String multisigHex = multisigWallet.makeMultisig(Arrays.asList(peers[0].getPreparedMultisigHex(), peers[1].getPreparedMultisigHex()), 2, tskWalletService.getWalletPassword()); // TODO (woodser): tskWalletService.makeMultisig(tradeId, multisigHexes, threshold)?
          ...
          if (trade.getSelf().getExchangedMultisigHex() == null && peers[0].getMadeMultisigHex() != null && peers[1].getMadeMultisigHex() != null) {
            ...
            MoneroMultisigInitResult result = multisigWallet.exchangeMultisigKeys(Arrays.asList(peers[0].getMadeMultisigHex(), peers[1].getMadeMultisigHex()), tskWalletService.getWalletPassword());
          ...
          if (processModel.getMultisigAddress() == null && peers[0].getExchangedMultisigHex() != null && peers[1].getExchangedMultisigHex() != null) {
            ...
            MoneroMultisigInitResult result = multisigWallet.exchangeMultisigKeys(Arrays.asList(peers[0].getExchangedMultisigHex(), peers[1].getExchangedMultisigHex()), tskWalletService.getWalletPassword());
