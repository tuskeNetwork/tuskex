...
public class ArbitratorProcessDepositRequest extends TradeTask {
    ...
    private void processDepositRequest() {
        ...
        // verify signature
        if (!TuskexUtils.isSignatureValid(peerPubKeyRing, contractAsJson, signature)) {
        ...
        MoneroTx verifiedTx;
        try {
            verifiedTx = trade.getTskWalletService().verifyDepositTx(
                    offer.getId(),
                    tradeFee,
        ...
        MoneroDaemon daemon = trade.getTskWalletService().getDaemon();
