...
public class ArbitratorProcessReserveTx extends TradeTask {
    ...
    protected void run() {
        try {
            ...
            BigInteger penaltyFee = TuskexUtils.multiply(isFromMaker ? offer.getAmount() : trade.getAmount(), offer.getPenaltyFeePct());
            ...
            MoneroTx verifiedTx;
            try {
                verifiedTx = trade.getTskWalletService().verifyReserveTx(
                    offer.getId(),
                    penaltyFee,
                    tradeFee,
                    sendAmount,
