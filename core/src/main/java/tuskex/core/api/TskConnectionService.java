...
import tuskex.core.tsk.nodes.TskNodesSetupPreferences;
...
@Slf4j
@Singleton
public final class TskConnectionService {
    ...
    @Inject
    public TskConnectionService(P2PService p2PService,
                                        Config config,
    ...
    public MoneroRpcConnection getBestAvailableConnection() {
        ...
        if (tskLocalNode.shouldBeIgnored() && connectionManager.hasConnection(tskLocalNode.getUri())) ignoredConnections.add(connectionManager.getConnectionByUri(tskLocalNode.getUri()));
    ...
    private boolean isConnectionLocalHost(MoneroRpcConnection connection) {
        return connection != null && TuskexUtils.isLocalHost(connection.getUri());
    }

    private long getDefaultRefreshPeriodMs() {
        MoneroRpcConnection connection = getConnection();
        if (connection == null) return TskLocalNode.REFRESH_PERIOD_LOCAL_MS;
        if (isConnectionLocalHost(connection)) {
            ...
            else return TskLocalNode.REFRESH_PERIOD_LOCAL_MS; // TODO: announce faster refresh after done syncing
    ...
    private boolean isProxyApplied(MoneroRpcConnection connection) {
        ...
        return connection.isOnion() || (preferences.getUseTorForTsk().isUseTorForTsk() && !TuskexUtils.isPrivateIp(connection.getUri()));
    }
    ...
    private void initializeConnections() {
        synchronized (lock) {
            ...
            if (!isInitialized) {
                ...
                tskLocalNode.addListener(new TskLocalNodeListener() {
                    @Override
                    ...
                    public void onConnectionChanged(MoneroRpcConnection connection) {
                        ...
                        if (isShutDownStarted || !connectionManager.getAutoSwitch() || !accountService.isAccountOpen() ||
                            !connectionManager.hasConnection(connection.getUri()) || tskLocalNode.shouldBeIgnored()) return;

                        // check connection
                        boolean isConnected = false;
                        if (tskLocalNode.isConnected()) {
            ...
            if ("".equals(config.tskNode)) {
                ...
                if (coreContext.isApiUser()) {
                    ...
                    for (TskNode node : tskNodes.getAllTskNodes()) {
                    ...
                    // add default connections
                    for (TskNode node : tskNodes.selectPreferredNodes(new TskNodesSetupPreferences(preferences))) {
                        if (node.hasClearNetAddress()) {
                            MoneroRpcConnection connection = new MoneroRpcConnection(node.getAddress() + ":" + node.getPort()).setPriority(node.getPriority());
                ...
                if (isFixedConnection()) {
                    ...
                    if (!tskLocalNode.shouldBeIgnored() || !tskLocalNode.equalsUri(connectionList.getCurrentConnectionUri().get())) {
                ...
                MoneroRpcConnection connection = new MoneroRpcConnection(config.tskNode, config.tskNodeUsername, config.tskNodePassword).setPriority(1);
                if (isProxyApplied(connection)) connection.setProxyUri(getProxyUri());
    ...
    private void maybeStartLocalNode() {
        ...
        // skip if seed node
        if (TuskexUtils.isSeedNode()) return;

        // start local node if offline and used as last connection
        if (connectionManager.getConnection() != null && tskLocalNode.equalsUri(connectionManager.getConnection().getUri()) && !tskLocalNode.isDetected() && !tskLocalNode.shouldBeIgnored()) {
            try {
                log.info("Starting local node");
                tskLocalNode.startMoneroNode();
            } catch (Exception e) {
    ...
    private void onConnectionChanged(MoneroRpcConnection currentConnection) {
        ...
        log.info("TskConnectionService.onConnectionChanged() uri={}, connected={}", currentConnection == null ? null : currentConnection.getUri(), currentConnection == null ? "false" : isConnected);
        synchronized (listenerLock) {
        ...
        synchronized (pollLock) {
            ...
            try {
                ...
                try {
                    ...
                    if (lastInfo == null && "".equals(config.tskNode) && preferences.getMoneroNodesOption() == TskNodes.MoneroNodesOption.CUSTOM) {
                ...
                UserThread.execute(() -> {
                    ...
                    boolean isTestnet = Config.baseCurrencyNetwork() == BaseCurrencyNetwork.TSK_LOCAL;
    ...
    private boolean isFixedConnection() {
        return !"".equals(config.tskNode) || preferences.getMoneroNodesOption() == TskNodes.MoneroNodesOption.CUSTOM;
    }
}
