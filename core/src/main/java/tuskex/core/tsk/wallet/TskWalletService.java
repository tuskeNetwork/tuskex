...
import tuskex.core.api.AccountServiceListener;
...
import tuskex.core.tsk.setup.DownloadListener;
import tuskex.core.tsk.setup.MoneroWalletRpcManager;
...
public class TskWalletService {
    private static final Logger log = LoggerFactory.getLogger(TskWalletService.class);
    ...
    public static final String MONERO_WALLET_RPC_NAME = Utilities.isWindows() ? "tuske-wallet-rpc.exe" : "tuske-wallet-rpc";
    ...
    private static final String MONERO_WALLET_RPC_USERNAME = "tuskex_user";
    private static final String MONERO_WALLET_RPC_DEFAULT_PASSWORD = "password"; // only used if account password is null
    private static final String MONERO_WALLET_NAME = "tuskex_TSK";
    ...
    private static final String THREAD_ID = TskWalletService.class.getSimpleName();
    ...
    private final TskAddressEntryList tskAddressEntryList;
    private final WalletsSetup walletsSetup;
    private final DownloadListener downloadListener = new DownloadListener();
    private final LongProperty walletHeight = new SimpleLongProperty(0);

    private final File walletDir;
    private final File tskWalletFile;
    private final int rpcBindPort;
    private final boolean useNativeTskWallet;
    protected final CopyOnWriteArraySet<TskBalanceListener> balanceListeners = new CopyOnWriteArraySet<>();
    ...
    @Inject
    TskWalletService(User user,
                     Preferences preferences,
                     CoreAccountService accountService,
                     TskConnectionService tskConnectionService,
                     WalletsSetup walletsSetup,
                     TskAddressEntryList tskAddressEntryList,
                     @Named(Config.WALLET_DIR) File walletDir,
                     @Named(Config.WALLET_RPC_BIND_PORT) int rpcBindPort,
                     @Named(Config.USE_NATIVE_TSK_WALLET) boolean useNativeTskWallet) {
        this.user = user;
        this.preferences = preferences;
        this.accountService = accountService;
        this.tskConnectionService = tskConnectionService;
        this.walletsSetup = walletsSetup;
        this.tskAddressEntryList = tskAddressEntryList;
        this.walletDir = walletDir;
        this.rpcBindPort = rpcBindPort;
        this.useNativeTskWallet = useNativeTskWallet;
        this.tskWalletFile = new File(walletDir, MONERO_WALLET_NAME);
        TuskexUtils.tskWalletService = this;
    ...
    public boolean isSyncedWithinTolerance() {
        if (!tskConnectionService.isSyncedWithinTolerance()) return false;
        Long targetHeight = tskConnectionService.getTargetHeight();
    ...
    public MoneroDaemonRpc getDaemon() {
        return tskConnectionService.getDaemon();
    }

    public TskConnectionService getConnectionService() {
        return tskConnectionService;
    }
    ...
    public boolean isProxyApplied(boolean wasWalletSynced) {
        return preferences.isProxyApplied(wasWalletSynced) && tskConnectionService.isProxyApplied();
    }
    ...
    private boolean isNativeLibraryApplied() {
        return useNativeTskWallet && MoneroUtils.isNativeLibraryLoaded();
    }
    ...
    public MoneroSyncResult syncWallet(MoneroWallet wallet) {
        synchronized (TuskexUtils.getDaemonLock()) { // TODO: lock defeats purpose of thread pool
    ...
    public MoneroTxWallet createTx(List<MoneroDestination> destinations) {
        ...
        //printTxs("TskWalletService.createTx", tx);
        return tx;
    }
    ...
    public MoneroTxWallet createReserveTx(BigInteger penaltyFee, BigInteger tradeFee, BigInteger sendTradeAmount, BigInteger securityDeposit, String returnAddress, boolean reserveExactAmount, Integer preferredSubaddressIndex) {
        synchronized (WALLET_LOCK) {
            synchronized (TuskexUtils.getWalletFunctionLock()) {
                ...
                MoneroTxWallet reserveTx = createTradeTx(penaltyFee, TuskexUtils.getBurnAddress(), sendAmount, returnAddress, reserveExactAmount, preferredSubaddressIndex);
    ...
    private MoneroTxWallet createTradeTxFromSubaddress(BigInteger feeAmount, String feeAddress, BigInteger sendAmount, String sendAddress, Integer subaddressIndex) {
        ...
        MoneroTxConfig txConfig = new MoneroTxConfig()
                ...
                .setPriority(TskWalletService.PROTOCOL_FEE_PRIORITY);
    ...
    public MoneroTx verifyReserveTx(String offerId, BigInteger penaltyFee, BigInteger tradeFee, BigInteger sendTradeAmount, BigInteger securityDeposit, String returnAddress, String txHash, String txHex, String txKey, List<String> keyImages) {
        BigInteger sendAmount = sendTradeAmount.add(securityDeposit).add(tradeFee).subtract(penaltyFee);
        return verifyTradeTx(offerId, penaltyFee, TuskexUtils.getBurnAddress(), sendAmount, returnAddress, txHash, txHex, txKey, keyImages);
    }
    ...
    public List<MoneroTx> getDaemonTxs(List<String> txHashes) {
        synchronized (txCache) {
            ...
            // fetch txs
            if (getDaemon() == null) tskConnectionService.verifyConnection(); // will throw
            ...
            }, tskConnectionService.getRefreshPeriodMs() / 1000);
            return txs;
        }
    }
    ...
    public void onShutDownStarted() {
        log.info("TskWalletService.onShutDownStarted()");
        ...
        Runnable shutDownTask = () -> {
            ...
            if (wallet != null) {
                try {
                    ...
                    log.warn("Error closing main wallet: {}. Was Tuskex stopped manually with ctrl+c?", e.getMessage());
                }
            }
        };
    ...
    public synchronized TskAddressEntry getNewAddressEntry() {
        return getNewAddressEntryAux(null, TskAddressEntry.Context.AVAILABLE);
    }

    public synchronized TskAddressEntry getNewAddressEntry(String offerId, TskAddressEntry.Context context) {

        // try to use available and not yet used entries
        try {
            List<TskAddressEntry> unusedAddressEntries = getUnusedAddressEntries();
            if (!unusedAddressEntries.isEmpty()) return tskAddressEntryList.swapAvailableToAddressEntryWithOfferId(unusedAddressEntries.get(0), context, offerId);
        } catch (Exception e) {
    ...
    private TskAddressEntry getNewAddressEntryAux(String offerId, TskAddressEntry.Context context) {
        MoneroSubaddress subaddress = wallet.createSubaddress(0);
        TskAddressEntry entry = new TskAddressEntry(subaddress.getIndex(), subaddress.getAddress(), context, offerId, null);
        log.info("Add new TskAddressEntry {}", entry);
        tskAddressEntryList.addAddressEntry(entry);
        return entry;
    }

    public synchronized TskAddressEntry getFreshAddressEntry() {
        List<TskAddressEntry> unusedAddressEntries = getUnusedAddressEntries();
        if (unusedAddressEntries.isEmpty()) return getNewAddressEntry();
        else return unusedAddressEntries.get(0);
    }

    public synchronized TskAddressEntry recoverAddressEntry(String offerId, String address, TskAddressEntry.Context context) {
        var available = findAddressEntry(address, TskAddressEntry.Context.AVAILABLE);
        if (!available.isPresent()) return null;
        return tskAddressEntryList.swapAvailableToAddressEntryWithOfferId(available.get(), context, offerId);
    }

    public synchronized TskAddressEntry getOrCreateAddressEntry(String offerId, TskAddressEntry.Context context) {
        Optional<TskAddressEntry> addressEntry = getAddressEntryListAsImmutableList().stream().filter(e -> offerId.equals(e.getOfferId())).filter(e -> context == e.getContext()).findAny();
        if (addressEntry.isPresent()) return addressEntry.get();
        else return getNewAddressEntry(offerId, context);
    }

    public synchronized Optional<TskAddressEntry> getAddressEntry(String offerId, TskAddressEntry.Context context) {
        List<TskAddressEntry> entries = getAddressEntryListAsImmutableList().stream().filter(e -> offerId.equals(e.getOfferId())).filter(e -> context == e.getContext()).collect(Collectors.toList());
    ...
    public synchronized void swapAddressEntryToAvailable(String offerId, TskAddressEntry.Context context) {
        Optional<TskAddressEntry> addressEntryOptional = getAddressEntryListAsImmutableList().stream().filter(e -> offerId.equals(e.getOfferId())).filter(e -> context == e.getContext()).findAny();
        addressEntryOptional.ifPresent(e -> {
            ...
            tskAddressEntryList.swapToAvailable(e);
    ...
    public synchronized void resetAddressEntriesForOpenOffer(String offerId) {
        ...
        swapAddressEntryToAvailable(offerId, TskAddressEntry.Context.OFFER_FUNDING);

        // swap trade payout to available if applicable
        if (tradeManager == null) return;
        Trade trade = tradeManager.getTrade(offerId);
        if (trade == null || trade.isPayoutUnlocked()) swapAddressEntryToAvailable(offerId, TskAddressEntry.Context.TRADE_PAYOUT);
    }

    public synchronized void resetAddressEntriesForTrade(String offerId) {
        swapAddressEntryToAvailable(offerId, TskAddressEntry.Context.TRADE_PAYOUT);
    }

    private Optional<TskAddressEntry> findAddressEntry(String address, TskAddressEntry.Context context) {
    ...
    public List<TskAddressEntry> getAddressEntries() {
        return getAddressEntryListAsImmutableList().stream().collect(Collectors.toList());
    }

    public List<TskAddressEntry> getAvailableAddressEntries() {
        return getAddressEntryListAsImmutableList().stream().filter(addressEntry -> TskAddressEntry.Context.AVAILABLE == addressEntry.getContext()).collect(Collectors.toList());
    }

    public List<TskAddressEntry> getAddressEntriesForOpenOffer() {
        return getAddressEntryListAsImmutableList().stream()
                .filter(addressEntry -> TskAddressEntry.Context.OFFER_FUNDING == addressEntry.getContext())
                .collect(Collectors.toList());
    }

    public List<TskAddressEntry> getAddressEntriesForTrade() {
        return getAddressEntryListAsImmutableList().stream()
                .filter(addressEntry -> TskAddressEntry.Context.TRADE_PAYOUT == addressEntry.getContext())
                .collect(Collectors.toList());
    }

    public List<TskAddressEntry> getAddressEntries(TskAddressEntry.Context context) {
    ...
    public TskAddressEntry getBaseAddressEntry() {
        return getAddressEntryListAsImmutableList().stream().filter(e -> e.getContext() == TskAddressEntry.Context.BASE_ADDRESS).findAny().orElse(null);
    }

    public List<TskAddressEntry> getFundedAvailableAddressEntries() {
    ...
    public List<TskAddressEntry> getAddressEntryListAsImmutableList() {
        for (MoneroSubaddress subaddress : cachedSubaddresses) {
            boolean exists = tskAddressEntryList.getAddressEntriesAsListImmutable().stream().filter(addressEntry -> addressEntry.getAddressString().equals(subaddress.getAddress())).findAny().isPresent();
            if (!exists) {
                TskAddressEntry entry = new TskAddressEntry(subaddress.getIndex(), subaddress.getAddress(), subaddress.getIndex() == 0 ? TskAddressEntry.Context.BASE_ADDRESS : TskAddressEntry.Context.AVAILABLE, null, null);
                tskAddressEntryList.addAddressEntry(entry);
            }
        }
        return tskAddressEntryList.getAddressEntriesAsListImmutable();
    }

    public List<TskAddressEntry> getUnusedAddressEntries() {
        return getAvailableAddressEntries().stream()
                .filter(e -> e.getContext() == TskAddressEntry.Context.AVAILABLE && !subaddressHasIncomingTransfers(e.getSubaddressIndex()))
                .collect(Collectors.toList());
    }
    ...
    public Stream<TskAddressEntry> getAddressEntriesForAvailableBalanceStream() {
        Stream<TskAddressEntry> available = getFundedAvailableAddressEntries().stream();
        available = Stream.concat(available, getAddressEntries(TskAddressEntry.Context.ARBITRATOR).stream());
        available = Stream.concat(available, getAddressEntries(TskAddressEntry.Context.OFFER_FUNDING).stream().filter(entry -> !tradeManager.getOpenOfferManager().getOpenOfferById(entry.getOfferId()).isPresent()));
        available = Stream.concat(available, getAddressEntries(TskAddressEntry.Context.TRADE_PAYOUT).stream().filter(entry -> tradeManager.getTrade(entry.getOfferId()) == null || tradeManager.getTrade(entry.getOfferId()).isPayoutUnlocked()));
    ...
    public void addBalanceListener(TskBalanceListener listener) {
        if (!balanceListeners.contains(listener)) balanceListeners.add(listener);
    }

    public void removeBalanceListener(TskBalanceListener listener) {
        balanceListeners.remove(listener);
    }

    public void updateBalanceListeners() {
        BigInteger availableBalance = getAvailableBalance();
        for (TskBalanceListener balanceListener : balanceListeners) {
            BigInteger balance;
    ...
    public void saveAddressEntryList() {
        tskAddressEntryList.requestPersistence();
    }
    ...
    private void initialize() {
        ...
        if (useNativeTskWallet && !MoneroUtils.isNativeLibraryLoaded()) {
            try {
        ...
        if (useNativeTskWallet && !isNativeLibraryApplied()) log.warn(appliedMsg);
        ...
        tskConnectionService.addConnectionListener(connection -> {
            ...
            if (!wasWalletSynced) {
                if (!Boolean.TRUE.equals(tskConnectionService.isConnected())) return;
                ThreadUtils.submitToPool(() -> {
        ...
        tskConnectionService.downloadPercentageProperty().addListener(walletInitListener);
    ...
    private void initMainWalletIfConnected() {
        ThreadUtils.execute(() -> {
            synchronized (WALLET_LOCK) {
                if (wallet == null && tskConnectionService.downloadPercentageProperty().get() == 1 && !isShutDownStarted) {
                    maybeInitMainWallet(true);
                    if (walletInitListener != null) tskConnectionService.downloadPercentageProperty().removeListener(walletInitListener);
                }
            }
        }, THREAD_ID);
    }
    ...
    private void maybeInitMainWallet(boolean sync) {
        try {
            ...
            e.printStackTrace();
            TuskexUtils.tuskexSetup.getTopErrorMsg().set(e.getMessage());
            throw e;
        }
    }
            ...
            if (wallet != null && !isShutDownStarted) {
                ...
                if (sync && numAttempts > 0) {
                    try {
                        ...
                        onConnectionChanged(tskConnectionService.getConnection());
                        ...
                        TuskexUtils.tuskexSetup.getWalletInitialized().set(true);

                        // save but skip backup on initialization
                        saveMainWallet(false);
                    } catch (Exception e) {
                        if (isClosingWallet || isShutDownStarted || TuskexUtils.tuskexSetup.getWalletInitialized().get()) return; // ignore if wallet closing, shut down started, or app already initialized
                        ...
                        if (numAttempts <= 1) {
                            ...
                            log.warn("Trying again in {} seconds", tskConnectionService.getRefreshPeriodMs() / 1000);
                            UserThread.runAfter(() -> {
    ...
    private void resetIfWalletChanged() {
        ...
        List<TskAddressEntry> baseAddresses = getAddressEntries(TskAddressEntry.Context.BASE_ADDRESS);
        if (baseAddresses.size() > 1 || (baseAddresses.size() == 1 && !baseAddresses.get(0).getAddressString().equals(wallet.getPrimaryAddress()))) {
            ...
            TuskexUtils.tuskexSetup.getTopErrorMsg().set(warningMsg);

            // reset address entries
            tskAddressEntryList.clear();
    ...
    private void syncWithProgress() {
        ...
        if (wallet instanceof MoneroWalletFull) {
            if (runReconnectTestOnStartup) TuskexUtils.waitFor(1000); // delay sync to test
        ...
        wallet.startSyncing(tskConnectionService.getRefreshPeriodMs());
        ...
        syncWithProgressLooper = new TaskLooper(() -> {
            ...
            if (height < tskConnectionService.getTargetHeight()) updateSyncProgress(height);
            else {
        ...
        TuskexUtils.awaitLatch(syncWithProgressLatch);
        wallet.stopSyncing();
    ...
    private void updateSyncProgress(long height) {
        UserThread.execute(() -> {
            ...
            if (height == 1) {
                downloadListener.progress(.0001, tskConnectionService.getTargetHeight() - height, null); // >0% shows progress bar
                return;
            }
    ...
    private MoneroWalletFull createWalletFull(MoneroWalletConfig config) {
        ...
        if (!Boolean.TRUE.equals(tskConnectionService.isConnected())) throw new RuntimeException("Must be connected to daemon before creating wallet");
        ...
        try {
            ...
            // create wallet
            MoneroRpcConnection connection = tskConnectionService.getConnection();
    ...
    private MoneroWalletFull openWalletFull(MoneroWalletConfig config, boolean applyProxyUri) {
        ...
        try {
            ...
            MoneroRpcConnection connection = new MoneroRpcConnection(tskConnectionService.getConnection());
    ...
    private MoneroWalletRpc createWalletRpc(MoneroWalletConfig config, Integer port) {
        ...
        try {
            ...
            throw new IllegalStateException("Could not create wallet '" + config.getPath() + "'. Please close Tuskex, stop all monero-wallet-rpc processes, and restart Tuskex.");
        }
    }
    ...
    private MoneroWalletRpc openWalletRpc(MoneroWalletConfig config, Integer port, boolean applyProxyUri) {
        ...
        try {
            ...
            e.printStackTrace();
            if (walletRpc != null) forceCloseWallet(walletRpc, config.getPath());
            throw new IllegalStateException("Could not open wallet '" + config.getPath() + "'. Please close Tuskex, stop all monero-wallet-rpc processes, and restart Tuskex.\n\nError message: " + e.getMessage());
        }
    }
    ...
    private void onConnectionChanged(MoneroRpcConnection connection) {
        synchronized (WALLET_LOCK) {
            ...
            if (TuskexUtils.connectionConfigsEqual(connection, wallet.getDaemonConnection())) return;
    ...
    private long getPollPeriod() {
        return tskConnectionService.getRefreshPeriodMs();
    }
    ...
    private void doPollWallet(boolean updateTxs) {
        synchronized (pollLock) {
            ...
            try {
                ...
                MoneroDaemonInfo lastInfo = tskConnectionService.getLastInfo();
                if (lastInfo == null) {
                    log.warn("Last daemon info is null");
                    return;
                }
                if (!tskConnectionService.isSyncedWithinTolerance()) {
                    log.warn("Monero daemon is not synced within tolerance, height={}, targetHeight={}", tskConnectionService.chainHeightProperty().get(), tskConnectionService.getTargetHeight());
                    return;
                }

                // switch to best connection if wallet is too far behind
                if (wasWalletSynced && walletHeight.get() < tskConnectionService.getTargetHeight() - NUM_BLOCKS_BEHIND_TOLERANCE && !Config.baseCurrencyNetwork().isTestnet()) {
                    log.warn("Updating connection because main wallet is {} blocks behind monerod, wallet height={}, monerod height={}", tskConnectionService.getTargetHeight() - walletHeight.get(), walletHeight.get(), lastInfo.getHeight());
                    tskConnectionService.switchToBestConnection();
                }

                // sync wallet if behind daemon
                if (walletHeight.get() < tskConnectionService.getTargetHeight()) {
                ...
                if (updateTxs) {
                    synchronized (WALLET_LOCK) { // avoid long fetch from blocking other operations
                        synchronized (TuskexUtils.getDaemonLock()) {
                            try {
