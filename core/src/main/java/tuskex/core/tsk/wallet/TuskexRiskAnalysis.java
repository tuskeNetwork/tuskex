...
// with 0 value.
// For Tuskex's use cases RBF is not considered risky. Requiring a confirmation for RBF payments from a user's
// external wallet to Tuskex would hurt usability. The trade transaction requires anyway a confirmation and we don't see
// a use case where a Tuskex user accepts unconfirmed payment from untrusted peers and would not wait anyway for at least
// one confirmation.
 ...
 * transactions.</p>
 */
public class TuskexRiskAnalysis implements RiskAnalysis {
    private static final Logger log = LoggerFactory.getLogger(TuskexRiskAnalysis.class);
    ...
    private TuskexRiskAnalysis(Wallet wallet, Transaction tx, List<Transaction> dependencies) {
        this.tx = tx;
    ...
    public static class Analyzer implements RiskAnalysis.Analyzer {
        @Override
        public TuskexRiskAnalysis create(Wallet wallet, Transaction tx, List<Transaction> dependencies) {
            return new TuskexRiskAnalysis(wallet, tx, dependencies);
        }
    }
