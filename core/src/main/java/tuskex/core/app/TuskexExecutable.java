...
import tuskex.common.config.ConfigException;
...
import tuskex.common.setup.CommonSetup;
...
import tuskex.core.setup.CorePersistedDataHost;
import tuskex.core.setup.CoreSetup;
...
@Slf4j
public abstract class TuskexExecutable implements GracefulShutDownHandler, TuskexSetup.TuskexSetupListener, UncaughtExceptionHandler {

    public static final String DEFAULT_APP_NAME = "Tuskex";
    ...
    public TuskexExecutable(String fullName, String scriptName, String appName, String version) {
    ...
    public int execute(String[] args) {
        try {
            ...
            if (config.helpRequested) {
                config.printHelp(System.out, new TuskexHelpFormatter(fullName, scriptName, version));
            ...
            System.err.println("fault: An unexpected error occurred. " +
                    "Please file a report at https://tuskex.exchange/issues");
    ...
    protected void onApplicationLaunched() {
        ...
        // We call startApplication to enable UI to show popup. We prevent in TuskexSetup to go further
        // in the process and require a shut down.
        isReadOnly = TuskexSetup.hasDowngraded();
    ...
    protected CompletableFuture<Boolean> loginAccount() {
        ...
        if (accountService.accountExists()) {
            ...
            log.info("Creating Tuskex account with null password");
    ...
    protected void runTuskexSetup() {
        TuskexSetup tuskexSetup = injector.getInstance(TuskexSetup.class);
    ...
    public void gracefulShutDown(ResultHandler onShutdown, boolean systemExit) {
        ...
        try {
            ...
            tasks.add(() -> injector.getInstance(TskWalletService.class).onShutDownStarted());
            tasks.add(() -> injector.getInstance(TskConnectionService.class).onShutDownStarted());
            ...
            injector.getInstance(OpenOfferManager.class).shutDown(() -> {
                ...
                injector.getInstance(P2PService.class).shutDown(() -> {
                    ...
                    injector.getInstance(TskWalletService.class).shutDown();
                    injector.getInstance(TskConnectionService.class).shutDown();
