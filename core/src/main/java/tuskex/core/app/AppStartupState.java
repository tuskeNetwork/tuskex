...
public class AppStartupState {
    ...
    public AppStartupState(CoreNotificationService notificationService,
        ...
        tskConnectionService.downloadPercentageProperty().addListener((observable, oldValue, newValue) -> {
        ...
        tskWalletService.downloadPercentageProperty().addListener((observable, oldValue, newValue) -> {
            if (tskWalletService.isDownloadComplete())
                isWalletSynced.set(true);
        });

        tskConnectionService.numPeersProperty().addListener((observable, oldValue, newValue) -> {
