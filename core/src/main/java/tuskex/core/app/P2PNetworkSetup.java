...
public class P2PNetworkSetup {
    ...
    BooleanProperty init(Runnable onReadyHandler, @Nullable Consumer<Boolean> displayTorNetworkSettingsHandler) {
        ...
        p2PNetworkInfoBinding = EasyBind.combine(bootstrapState, bootstrapWarning, p2PService.getNumConnectedPeers(),
                tskConnectionService.numPeersProperty(), hiddenServicePublished, initialP2PNetworkDataReceived,
                (state, warning, numP2pPeers, numTskPeers, hiddenService, dataReceived) -> {
                    String result;
                    int p2pPeers = (int) numP2pPeers;
                    if (warning != null && p2pPeers == 0) {
                        result = warning;
                    } else {
                        String p2pInfo = ((int) numTskPeers > 0 ? Res.get("mainView.footer.tskPeers", numTskPeers) + " / " : "") + Res.get("mainView.footer.p2pPeers", numP2pPeers);
