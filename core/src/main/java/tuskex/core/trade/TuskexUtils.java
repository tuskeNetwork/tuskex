...
public class TuskexUtils {
    ...
    public static int TSK_SMALLEST_UNIT_EXPONENT = 12;
    ...
    private static final BigInteger TSK_AU_MULTIPLIER = new BigInteger("1000000000000");
    public static final DecimalFormat TSK_FORMATTER = new DecimalFormat("##############0.000000000000", DECIMAL_FORMAT_SYMBOLS);
    public static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");

    // TODO: better way to share references?
    public static TuskexSetup tuskexSetup;
    public static ArbitrationManager arbitrationManager;
    public static TskWalletService tskWalletService;
    public static OpenOfferManager openOfferManager;

    public static boolean isSeedNode() {
        return tuskexSetup == null;
    }
    ...
    public static double centinerosToTsk(long centineros) {
        return atomicUnitsToTsk(centinerosToAtomicUnits(centineros));
    }
    ...
    public static double atomicUnitsToTsk(long atomicUnits) {
        return atomicUnitsToTsk(BigInteger.valueOf(atomicUnits));
    }

    public static double atomicUnitsToTsk(BigInteger atomicUnits) {
        return new BigDecimal(atomicUnits).divide(new BigDecimal(TSK_AU_MULTIPLIER)).doubleValue();
    }

    public static BigInteger tskToAtomicUnits(double tsk) {
        return new BigDecimal(tsk).multiply(new BigDecimal(TSK_AU_MULTIPLIER)).toBigInteger();
    }

    public static long tskToCentineros(double tsk) {
        return atomicUnitsToCentineros(tskToAtomicUnits(tsk));
    }

    public static double coinToTsk(Coin coin) {
        return atomicUnitsToTsk(coinToAtomicUnits(coin));
    }

    public static double divide(BigInteger auDividend, BigInteger auDivisor) {
        return atomicUnitsToTsk(auDividend) / atomicUnitsToTsk(auDivisor);
    }
    ...
    public static String formatTsk(BigInteger atomicUnits) {
        return formatTsk(atomicUnits, false);
    }

    public static String formatTsk(BigInteger atomicUnits, int decimalPlaces) {
        return formatTsk(atomicUnits, false, decimalPlaces);
    }

    public static String formatTsk(BigInteger atomicUnits, boolean appendCode) {
        return formatTsk(atomicUnits, appendCode, 0);
    }

    public static String formatTsk(BigInteger atomicUnits, boolean appendCode, int decimalPlaces) {
        if (atomicUnits == null) return "";
        return formatTsk(atomicUnits.longValueExact(), appendCode, decimalPlaces);
    }

    public static String formatTsk(long atomicUnits) {
        return formatTsk(atomicUnits, false, 0);
    }

    public static String formatTsk(long atomicUnits, boolean appendCode) {
        return formatTsk(atomicUnits, appendCode, 0);
    }

    public static String formatTsk(long atomicUnits, boolean appendCode, int decimalPlaces) {
        String formatted = TSK_FORMATTER.format(atomicUnitsToTsk(atomicUnits));
        ...
        return applyDecimals(formatted, Math.max(2, decimalPlaces)) + (appendCode ? " TSK" : "");
    }
    ...
    public static BigInteger parseTsk(String input) {
        if (input == null || input.length() == 0) return BigInteger.ZERO;
        try {
            return new BigDecimal(input).multiply(new BigDecimal(TSK_AU_MULTIPLIER)).toBigInteger();
        } catch (Exception e) {
    ...
    public static byte[] signOffer(OfferPayload offer, KeyRing keyRing) {
        return TuskexUtils.sign(keyRing, offer.getSignatureHash());
    }
    ...
    public static int getDefaultMoneroPort() {
        if (Config.baseCurrencyNetwork().isMainnet()) return 20241;
        else if (Config.baseCurrencyNetwork().isTestnet()) return 30241;
        else if (Config.baseCurrencyNetwork().isStagenet()) return 40241;
