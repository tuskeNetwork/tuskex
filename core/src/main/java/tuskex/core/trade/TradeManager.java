...
import tuskex.core.trade.Trade.DisputeState;
import tuskex.core.trade.Trade.Phase;
...
import tuskex.core.trade.protocol.ArbitratorProtocol;
import tuskex.core.trade.protocol.MakerProtocol;
import tuskex.core.trade.protocol.ProcessModel;
import tuskex.core.trade.protocol.ProcessModelServiceProvider;
import tuskex.core.trade.protocol.TakerProtocol;
import tuskex.core.trade.protocol.TradeProtocol;
import tuskex.core.trade.protocol.TradeProtocolFactory;
import tuskex.core.trade.protocol.TraderProtocol;
...
public class TradeManager implements PersistedDataHost, DecryptedDirectMessageListener {
    ...
    public TradeManager(User user,
        ...
        tskWalletService.setTradeManager(this);
    }
    ...
    private void initPersistedTrades() {
        ...
        new Thread(() -> {
            ...
            if (!TuskexUtils.isSeedNode()) {
                ...
                tskWalletService.fixReservedOutputs();

                // reset any available funded address entries
                if (isShutDownStarted) return;
                tskWalletService.getAddressEntriesForAvailableBalanceStream()
                        .filter(addressEntry -> addressEntry.getOfferId() != null)
                        .forEach(addressEntry -> {
                            log.warn("Swapping pending {} entries at startup. offerId={}", addressEntry.getContext(), addressEntry.getOfferId());
                            tskWalletService.swapAddressEntryToAvailable(addressEntry.getOfferId(), addressEntry.getContext());
                        });
    ...
    public void onTakeOffer(BigInteger amount,
        ...
        ((TakerProtocol) tradeProtocol).onTakeOffer(result -> {
            ...
            tskWalletService.resetAddressEntriesForOpenOffer(trade.getId()); // TODO: move to maybe remove on error
            trade.onProtocolError();
    ...
    public void onWithdrawRequest(String toAddress,
        ...
        throw new RuntimeException("Withdraw trade funds after payout to Tuskex wallet not supported");
    }
    ...
    public void onTradeCompleted(Trade trade) {
        ...
        tskWalletService.resetAddressEntriesForTrade(trade.getId());
        requestPersistence();
    }
    ...
    private boolean recoverAddresses(Trade trade) {
        ...
        tskWalletService.recoverAddressEntry(trade.getId(), entries.second,
                TskAddressEntry.Context.TRADE_PAYOUT);
        return true;
    }
