...
import tuskex.core.support.dispute.refund.RefundResultState;
...
import tuskex.core.trade.protocol.ProcessModelServiceProvider;
import tuskex.core.trade.protocol.TradeListener;
...
public abstract class Trade implements Tradable, Model {
    ...
    private static final String MONERO_TRADE_WALLET_PREFIX = "tsk_trade_";
    ...
    // Immutable
    @Getter
    transient final private TskWalletService tskWalletService;
    @Getter
    transient final private TskConnectionService tskConnectionService;
    ...
    @Setter
    private long startTime; // added for tuskex
    @Getter
    @Nullable
    ...
    // Added at v1.3.8
    // We use that for the TSK txKey but want to keep it generic to be flexible for other payment methods or assets.
    @Getter
    @Setter
    private String counterCurrencyExtraData;

    // Added in TSK integration
    ...
    protected Trade(Offer offer,
        ...
        this.tskConnectionService = tskWalletService.getConnectionService();
    ...
    public void initialize(ProcessModelServiceProvider serviceProvider) {
            ...
            ThreadUtils.submitToPool(() -> {
                ...
                if (newValue == Trade.PayoutState.PAYOUT_PUBLISHED) {
                    ...
                    if (!isPayoutConfirmed()) {
                        new Thread(() -> {
                            TuskexUtils.waitFor(1000);
                            if (isShutDownStarted) return;
                            if (tskConnectionService.isConnected()) syncAndPollWallet();
                        }).start();
                    }
                    ...
                    processModel.getTskWalletService().resetAddressEntriesForTrade(getId());
                }
        ...
        if (this instanceof ArbitratorTrade) {
            ...
            tskWalletService.addWalletListener(idlePayoutSyncer);
        }
        ...
        if (walletExists()) getWallet();
            ...
            if (payoutTx != null && payoutTx.getNumConfirmations() >= TskWalletService.NUM_BLOCKS_UNLOCK) {
    ...
    public void awaitInitialized() {
        while (!isFullyInitialized) TuskexUtils.waitFor(100); // TODO: use proper notification and refactor isInitialized, fullyInitialized, and arbitrator idling
    }
    ...
    public boolean walletExists() {
        synchronized (walletLock) {
            return tskWalletService.walletExists(getWalletName());
        }
    }
    ...
    public MoneroWallet createWallet() {
        synchronized (walletLock) {
            ...
            wallet = tskWalletService.createWallet(getWalletName());
    ...
    public MoneroWallet getWallet() {
        synchronized (walletLock) {
            ...
            else wallet = tskWalletService.openWallet(getWalletName(), tskWalletService.isProxyApplied(wasWalletSynced));
            return wallet;
        }
    }
    ...
    public void pollWalletNormallyForMs(long pollNormalDuration) {
        ...
        setPollPeriod(tskConnectionService.getRefreshPeriodMs());

        // reset wallet poll period after duration 
        new Thread(() -> {
            TuskexUtils.waitFor(pollNormalDuration);
    ...
    public void importMultisigHex() {
        synchronized (walletLock) {
            synchronized (TuskexUtils.getDaemonLock()) { // lock on daemon because import calls full refresh
    ...
    public void saveWallet() {
        synchronized (walletLock) {
            ...
            tskWalletService.saveWallet(wallet);
            maybeBackupWallet();
        }
    }
    ...
    private void maybeBackupWallet() {
        ...
        if (createBackup) tskWalletService.backupWallet(getWalletName());
    }
    ...
    private void closeWallet() {
        synchronized (walletLock) {
            ...
            stopPolling();
            tskWalletService.closeWallet(wallet, true);
            wallet = null;
            pollPeriodMs = null;
        }
    }
    ...
    private void forceCloseWallet() {
        if (wallet != null) {
            tskWalletService.forceCloseWallet(wallet, wallet.getPath());
            stopPolling();
            wallet = null;
        }
    }
        ...
        synchronized (walletLock) {
            if (walletExists()) {
                try {
                    ...
                    tskWalletService.deleteWallet(getWalletName());
                    tskWalletService.deleteWalletBackups(getWalletName());
                } catch (Exception e) {
    ...
    public Contract createContract() {
        ...
        Contract contract = new Contract(
                ...
                this instanceof MakerTrade ? tskWalletService.getAddressEntry(getId(), TskAddressEntry.Context.TRADE_PAYOUT).get().getAddressString() : getMaker().getPayoutAddressString(), // maker payout address
                this instanceof TakerTrade ? tskWalletService.getAddressEntry(getId(), TskAddressEntry.Context.TRADE_PAYOUT).get().getAddressString() : getTaker().getPayoutAddressString(), // taker payout address
    ...
    private MoneroTxWallet doCreatePayoutTx() {
        ...
        getBuyer().setPayoutAmount(TuskexUtils.getDestination(buyerPayoutAddress, payoutTx).getAmount());
        getSeller().setPayoutTxFee(payoutTxFeeSplit);
        getSeller().setPayoutAmount(TuskexUtils.getDestination(sellerPayoutAddress, payoutTx).getAmount());
    ...
    public void processPayoutTx(String payoutTxHex, boolean sign, boolean publish) {
        ...
        if (sign) {
            ...
            if (feeDiff > TskWalletService.MINER_FEE_TOLERANCE) throw new IllegalArgumentException("Miner fee is not within " + (TskWalletService.MINER_FEE_TOLERANCE * 100) + "% of estimated fee, expected " + feeEstimate + " but was " + payoutTx.getFee());
    ...
    public void shutDown() {
        ...
        Runnable shutDownTask = () -> {
            ...
            if (wallet != null) {
                try {
                    ...
                    //log.warn("Error closing monero-wallet-rpc subprocess for {} {}: {}. Was Tuskex stopped manually with ctrl+c?", getClass().getSimpleName(), getId(), e.getMessage());
                }
            }
        };
        ...
        if (idlePayoutSyncer != null) {
            tskWalletService.removeWalletListener(idlePayoutSyncer);
    ...
    public void onProtocolError() {
        ...
        if (this instanceof TakerTrade) {
            ThreadUtils.submitToPool(() -> {
                tskWalletService.thawOutputs(getSelf().getReserveTxKeyImages());
            });
        }
        ...
        if (processModel.getTradeProtocolErrorHeight() == 0) {
            log.warn("Scheduling to remove trade if unfunded for {} {} from height {}", getClass().getSimpleName(), getId(), tskConnectionService.getLastInfo().getHeight());
            processModel.setTradeProtocolErrorHeight(tskConnectionService.getLastInfo().getHeight());
        }
        ...
        protocolErrorHeightSubscription = EasyBind.subscribe(walletHeight, lastWalletHeight -> {
            ...
            ThreadUtils.submitToPool(() -> {
                ...
                MoneroTx makerDepositTx = getMaker().getDepositTxHash() == null ? null : tskWalletService.getDaemon().getTx(getMaker().getDepositTxHash());
                MoneroTx takerDepositTx = getTaker().getDepositTxHash() == null ? null : tskWalletService.getDaemon().getTx(getTaker().getDepositTxHash());
    ...
    private void restoreDepositsPublishedTrade() {
        ...
        // re-freeze outputs
        tskWalletService.freezeOutputs(getSelf().getReserveTxKeyImages());
    ...
    public List<Dispute> getDisputes() {
        return TuskexUtils.arbitrationManager.findDisputes(getId());
    }
    ...
    public MoneroTx getPayoutTx() {
        if (payoutTx == null) {
            payoutTx = payoutTxId == null ? null : (this instanceof ArbitratorTrade) ? tskWalletService.getDaemonTxWithCache(payoutTxId) : tskWalletService.getTx(payoutTxId);
        }
        return payoutTx;
    }
    ...
    private void setStartTimeFromUnlockedTxs() {
        ...
        MoneroDaemon daemonRpc = tskWalletService.getDaemon();
    ...
    public BigInteger getFrozenAmount() {
        ...
        if (getSelf().getReserveTxKeyImages() != null) {
            for (String keyImage : getSelf().getReserveTxKeyImages()) {
                List<MoneroOutputWallet> outputs = tskWalletService.getOutputs(new MoneroOutputQuery().setIsFrozen(true).setIsSpent(false).setKeyImage(new MoneroKeyImage(keyImage)));
    ...
    public long getReprocessDelayInSeconds(int reprocessCount) {
        ...
        if (reprocessCount < retryCycles) return tskConnectionService.getRefreshPeriodMs() / 1000;
        long delay = 60;
    ...
    private void onConnectionChanged(MoneroRpcConnection connection) {
        synchronized (walletLock) {
            ...
            if (tskWalletService.isProxyApplied(wasWalletSynced) && wallet instanceof MoneroWalletRpc && !StringUtils.equals(oldProxyUri, newProxyUri)) {
            ...
            if (isInitialized && connection != null && !Boolean.FALSE.equals(tskConnectionService.isConnected())) {
    ...
    private void tryInitPollingAux() {
        ...
        TuskexUtils.arbitrationManager.maybeReprocessDisputeClosedMessage(this, false);
    ...
    private void syncWallet(boolean pollWallet) {
        ...
        if (!wasWalletSynced) {
            ...
            if (tskWalletService.isProxyApplied(wasWalletSynced)) {
    ...
    private void doPollWallet() {
        synchronized (pollLock) {
            ...
            try {
                ...
                if (tskConnectionService.getTargetHeight() == null || !tskConnectionService.isSyncedWithinTolerance()) return;

                // sync if wallet too far behind daemon
                if (walletHeight.get() < tskConnectionService.getTargetHeight() - SYNC_EVERY_NUM_BLOCKS) syncWallet(false);
                ...
                if (!isDepositsUnlocked()) {
                    ...
                    if (getMaker().getDepositTx().getNumConfirmations() >= TskWalletService.NUM_BLOCKS_UNLOCK && getTaker().getDepositTx().getNumConfirmations() >= TskWalletService.NUM_BLOCKS_UNLOCK) {
                ...
                if (isConnectionRefused) forceRestartTradeWallet();
                    ...
                    if (!isShutDownStarted && wallet != null && isWalletConnected) {
                        log.warn("Error polling trade wallet for {} {}, errorMessage={}. Monerod={}", getClass().getSimpleName(), getShortId(), e.getMessage(), getTskWalletService().getConnectionService().getConnection());
                        //e.printStackTrace();
                    }
                }
            } finally {
    ...
    private void syncWalletIfBehind() {
        if (isWalletBehind()) {
            synchronized (walletLock) {
                tskWalletService.syncWallet(wallet);
    ...
    private boolean isWalletBehind() {
        return walletHeight.get() < tskConnectionService.getTargetHeight();
    }
            ...
            ThreadUtils.execute(() -> { // allow rapid notifications
                ...
                try {
                    ...
                    if (payoutHeight == null && getPayoutTxId() != null && isPayoutPublished()) {
                        MoneroTx tx = tskWalletService.getDaemon().getTx(getPayoutTxId());
                    ...
                    long currentHeight = tskWalletService.getDaemon().getHeight();
                    if (!isPayoutConfirmed() || (payoutHeight != null && currentHeight >= payoutHeight + TskWalletService.NUM_BLOCKS_UNLOCK)) {
                    ...
                    if (isWalletConnectedToDaemon()) {
                        e.printStackTrace();
                        log.warn("Error polling idle trade for {} {}: {}. Monerod={}", getClass().getSimpleName(), getId(), e.getMessage(), getTskWalletService().getConnectionService().getConnection());
                    };
                }
            }, getId());
        }
    ...
    private void onDepositsPublished() {
        ...
        if (this instanceof MakerTrade) {
            ...
            getTskWalletService().resetAddressEntriesForOpenOffer(getId());
        }

        // freeze outputs until spent
        ThreadUtils.submitToPool(() -> tskWalletService.freezeOutputs(getSelf().getReserveTxKeyImages()));
    }
