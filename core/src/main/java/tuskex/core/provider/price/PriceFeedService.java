...
import tuskex.core.provider.PriceHttpClient;
...
public class PriceFeedService {
    ...
    private void setTuskexMarketPrice(String currencyCode, Price price) {
        UserThread.execute(() -> {
            synchronized (cache) {
    ...
    public void applyLatestTuskexMarketPrice(Set<TradeStatistics3> tradeStatisticsSet) {
        ...
        mapByCurrencyCode.values().stream()
                ...
                .forEach(list -> {
                    ...
                    setTuskexMarketPrice(tradeStatistics.getCurrency(), tradeStatistics.getTradePrice());
                });
    }

    /**
     * Returns prices for all available currencies.
     * For crypto currencies the value is TSK price for 1 unit of given crypto currency (e.g. 1 DOGE = X TSK).
     * For traditional currencies the value is price in the given traditional currency per 1 TSK (e.g. 1 TSK = X USD).
     *
