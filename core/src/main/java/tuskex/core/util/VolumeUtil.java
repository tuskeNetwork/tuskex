...
import tuskex.core.monetary.CryptoExchangeRate;
import tuskex.core.monetary.Price;
import tuskex.core.monetary.TraditionalMoney;
import tuskex.core.monetary.TraditionalExchangeRate;
...
public class VolumeUtil {
    ...
    public static Volume getRoundedVolumePrecise(Volume volumeByAmount) {
        DecimalFormat decimalFormat = new DecimalFormat("#.####", TuskexUtils.DECIMAL_FORMAT_SYMBOLS);
    ...
    public static Volume getVolume(BigInteger amount, Price price) {
        ...
        if (price.getMonetary() instanceof CryptoMoney) {
            return new Volume(new CryptoExchangeRate((CryptoMoney) price.getMonetary()).coinToCrypto(TuskexUtils.atomicUnitsToCoin(amount)));
        } else {
            return new Volume(new TraditionalExchangeRate((TraditionalMoney) price.getMonetary()).coinToTraditionalMoney(TuskexUtils.atomicUnitsToCoin(amount)));
        }
    }
