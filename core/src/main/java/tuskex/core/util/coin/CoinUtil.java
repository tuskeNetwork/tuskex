...
import static tuskex.core.util.VolumeUtil.getAdjustedVolumeUnit;
...
public class CoinUtil {
    ...
    public static double getAsPercentPerBtc(BigInteger value) {
        return getAsPercentPerBtc(value, TuskexUtils.tskToAtomicUnits(1.0));
    }
    ...
    public static double getAsPercentPerBtc(BigInteger part, BigInteger total) {
        return MathUtils.roundDouble(TuskexUtils.divide(part == null ? BigInteger.ZERO : part, total == null ? BigInteger.valueOf(1) : total), 4);
    }
    ...
    public static BigInteger getRoundedAmount4Decimals(BigInteger amount, Price price, long maxTradeLimit) {
        ...
        double roundedTskAmount = Double.parseDouble(decimalFormat.format(TuskexUtils.atomicUnitsToTsk(amount)));
        return TuskexUtils.tskToAtomicUnits(roundedTskAmount);
    }
    ...
    static BigInteger getAdjustedAmount(BigInteger amount, Price price, long maxTradeLimit, int factor) {
        checkArgument(
                ...
                "amount needs to be above minimum of " + TuskexUtils.atomicUnitsToTsk(Restrictions.getMinTradeAmount()) + " tsk"
        );
        checkArgument(
                factor > 0,
        ...
        checkArgument(
                ...
                "MinTradeAmount must be at least " + TuskexUtils.atomicUnitsToTsk(Restrictions.getMinTradeAmount()) + " tsk"
        );
        ...
        long adjustedAmount = TuskexUtils.centinerosToAtomicUnits(Math.round(TuskexUtils.atomicUnitsToCentineros(amountByVolume) / 10000d) * 10000).longValueExact();
