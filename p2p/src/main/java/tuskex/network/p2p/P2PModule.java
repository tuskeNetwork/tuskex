...
import static tuskex.common.config.Config.BAN_LIST;
import static tuskex.common.config.Config.MAX_CONNECTIONS;
import static tuskex.common.config.Config.NODE_PORT;
import static tuskex.common.config.Config.REPUBLISH_MAILBOX_ENTRIES;
import static tuskex.common.config.Config.SOCKS_5_PROXY_HTTP_ADDRESS;
import static tuskex.common.config.Config.SOCKS_5_PROXY_TSK_ADDRESS;
import static tuskex.common.config.Config.TORRC_FILE;
import static tuskex.common.config.Config.TORRC_OPTIONS;
import static tuskex.common.config.Config.TOR_CONTROL_COOKIE_FILE;
import static tuskex.common.config.Config.TOR_CONTROL_HOST;
import static tuskex.common.config.Config.TOR_CONTROL_PASSWORD;
import static tuskex.common.config.Config.TOR_CONTROL_PORT;
import static tuskex.common.config.Config.TOR_CONTROL_USE_SAFE_COOKIE_AUTH;
import static tuskex.common.config.Config.TOR_DIR;
import static tuskex.common.config.Config.TOR_STREAM_ISOLATION;
import static tuskex.common.config.Config.USE_LOCALHOST_FOR_P2P;
import tuskex.network.Socks5ProxyProvider;
import tuskex.network.http.HttpClient;
...
import tuskex.network.p2p.peers.getdata.RequestDataManager;
import tuskex.network.p2p.peers.keepalive.KeepAliveManager;
import tuskex.network.p2p.peers.peerexchange.PeerExchangeManager;
...
public class P2PModule extends AppModule {
    ...
    protected void configure() {
        ...
        bindConstant().annotatedWith(named(SOCKS_5_PROXY_TSK_ADDRESS)).to(config.socks5ProxyTskAddress);
