...
export CLI_BASE="./tuskex-cli --password=xyz"
...
checksetup() {
    ...
    apitestusage() {
        ...
        echo "./tuskex-apitest --apiPassword=xyz --supportingApps=bitcoind,seednode,arbdaemon,alicedaemon,bobdaemon  --shutdownAfterTests=false"
        exit 1;
    }
    ...
    if [ -f "$APP_HOME/tuskex-cli" ]; then
        printdate "The tuskex-cli script exists.";
    else
        printdate "Error:  The tuskex-cli script not found, maybe because you are not running the script from the project root dir."
        exit 1
    fi
    printdate "Checking to see local bitcoind is running, and bitcoin-cli is in PATH."
    checkbitcoindrunning
    checkbitcoincliinpath
    printdate "Checking to see tuskex servers are running."
...
checkseednoderunning() {
    if [[ "$LINUX" == "TRUE" ]]; then
        if pgrep -f "tuskex.seednode.SeedNodeMain" > /dev/null ; then
...
checkarbnoderunning() {
    if [[ "$LINUX" == "TRUE" ]]; then
        if pgrep -f "tuskex.daemon.app.TuskexDaemonMain --appName=tuskex-TSK_STAGENET_Arb" > /dev/null ; then
        ...
        if ps -A | awk '/[b]isq.daemon.app.TuskexDaemonMain --appName=tuskex-TSK_STAGENET_Arb/ {print $1}' > /dev/null ; then
            printdate "The arbitration node is running on host."
        else
            printdate "Error:  arbitration node is not running on host, exiting."
            apitestusage
...
checkalicenoderunning() {
    if [[ "$LINUX" == "TRUE" ]]; then
        if pgrep -f "tuskex.daemon.app.TuskexDaemonMain --appName=tuskex-TSK_STAGENET_Alice" > /dev/null ; then
        ...
        if ps -A | awk '/[b]isq.daemon.app.TuskexDaemonMain --appName=tuskex-TSK_STAGENET_Alice/ {print $1}' > /dev/null ; then
