...
# Runs tsk <-> btc trading scenarios using the API CLI with a local regtest bitcoin node.
#
# Prerequisites:
#
#  - Linux or OSX with bash, Java 11-15 (JDK language compatibility 11), and bitcoin-core (v0.19 -  v22).
#
#  - Tuskex must be fully built with apitest dao setup files installed.
...
#    `$ ./tuskex-apitest --apiPassword=xyz --supportingApps=bitcoind,seednode,arbdaemon,alicedaemon,bobdaemon --shutdownAfterTests=false`
#
# Usage:
#
#  This script must be run from the root of the project, e.g.:
#
#     `$ apitest/scripts/trade-tsk-simulation.sh -d buy -f 0.05 -a 0.125`
#
#  Script options:  -d <direction> -m <mkt-price-margin(%)> -f <fixed-price> -a <amount(btc)>
#
# Examples:
#
#    Create a buy/tsk offer to buy 0.125 btc at an tsk fixed-price of 0.05 btc, using an tsk payment account:
#
#       `$ apitest/scripts/trade-tsk-simulation.sh -d buy -f 0.05 -a 0.125`
#
#    Create a sell/tsk offer to sell 0.125 btc at at an tsk mkt-price-margin of 0%, using using an tsk payment account:
#
#       `$ apitest/scripts/trade-tsk-simulation.sh -d sell -m 0.00 -a 0.125`

export APP_BASE_NAME=$(basename "$0")
export APP_HOME=$(pwd -P)
export APITEST_SCRIPTS_HOME="$APP_HOME/apitest/scripts"
export CURRENCY_CODE="TSK"
export ALICE_TSK_ADDRESS="44i8xZbd8ecaD6nQQrHjr1BwTp6QfGL22iWqHZKmU4QYSyr1F64XAxM4HgvQHxbny7ehfxemaA9LPDLz2wY3fxhB1bbMEco"
export BOB_TSK_ADDRESS="48xdBkXaCosPxcWwXRZdSGc33M9tYu6k9ga56dqkNrgsjQuJX16xW2qTyWTZstJpXXj87dj5p4H3y1xAfoVjAysoAYrXh2N"
...
# Demonstrate how to create an TSK payment account.

printdate "Create Alice's TSK Trading Payment Account."
# Note: Having problems passing a double quoted --account-name param to function.
CMD="$CLI_BASE --port=$ALICE_PORT createcryptopaymentacct --account-name=Alice_TSK_Account"
CMD+=" --currency-code=TSK --address=$ALICE_TSK_ADDRESS --trade-instant=false"
printdate "ALICE CLI: $CMD"
CMD_OUTPUT=$(createpaymentacct "$CMD")
echo "$CMD_OUTPUT"
printbreak
export ALICE_ACCT_ID=$(getnewpaymentacctid "$CMD_OUTPUT")
printdate "Alice's TSK payment-account-id: $ALICE_ACCT_ID"
exitoncommandalert $?
printbreak

printdate "Create Bob's TSK Trading Payment Account."
# Note: Having problems passing a double quoted --account-name param to function.
CMD="$CLI_BASE --port=$BOB_PORT createcryptopaymentacct --account-name=Bob_TSK_Account"
CMD+=" --currency-code=TSK --address=$BOB_TSK_ADDRESS --trade-instant=false"
printdate "BOB CLI: $CMD"
CMD_OUTPUT=$(createpaymentacct "$CMD")
echo "$CMD_OUTPUT"
printbreak
export BOB_ACCT_ID=$(getnewpaymentacctid "$CMD_OUTPUT")
printdate "Bob's TSK payment-account-id: $BOB_ACCT_ID"
exitoncommandalert $?
printbreak
